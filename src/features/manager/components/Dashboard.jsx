...
const StatCard = ({ label, value, color, icon }) => {
  ...
  return (
    <div className={`${t.card} rounded-xl p-4 flex flex-col gap-2.5`}>
      <div className={`${t.icon} w-8 h-8 rounded-lg flex items-center justify-center shrink-0`}>
        {icon}
      </div>
      <div>
...
const trophyColor = (i) => i === 0 ? "#BA7517" : i === 1 ? "#888780" : "#993C1D";
...
export const ManagerDashboard = () => {
  ...
  const selectedCount = status_distribution.find((s) => s.status === "selected")?.count ?? 0;
  const pendingCount  = status_distribution.find((s) => s.status === "pending")?.count  ?? 0;

  const barChartData = faculty_performance.map((f) => ({ name: f.name, contributions: f.total_contributions }));
  const pieChartData = faculty_performance.map((f) => ({ name: f.name, value: f.contributor_count }));
  ...
  const handleDownloadFaculty = async (facultyId, facultyName) => {
    ...
    try {
      ...
      triggerBlobDownload(res.data, `${facultyName.replace(/\s+/g, "_")}_contributions.zip`);
    } catch (err) {
      console.error("Faculty ZIP download failed:", err);
      toast.error("Failed to download faculty ZIP. This faculty may have no selected contributions.");
    } finally {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-xl font-medium text-foreground">University overview</h1>
          <p className="text-xs text-default-400 mt-0.5">Academic Year: {academic_year.name}</p>
        </div>
      </div>

      {/* ── Stat cards ── */}
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        <StatCard label="Total students"      value={total_students}            color="blue"   icon={<LuUsers size={14} />} />
        <StatCard label="Total contributions" value={total_contributions}        color="teal"   icon={<LuFileText size={14} />} />
        <StatCard label="Selected"            value={selectedCount}              color="green"  icon={<LuCheck size={14} />} />
        <StatCard label="Pending"             value={pendingCount}               color="amber"  icon={<LuClock size={14} />} />
        <StatCard label="Faculties"           value={faculty_performance.length} color="purple" icon={<LuBuilding2 size={14} />} />
      </div>

      {/* ── Charts row 1: Bar + Donut ── */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <Card className="border border-default-100 shadow-none">
          <CardHeader className="text-[13px] font-medium pb-1">Contributions by faculty</CardHeader>
        ...
        <Card className="border border-default-100 shadow-none">
          <CardHeader className="text-[13px] font-medium pb-1">Contributors by faculty</CardHeader>
          <CardBody className="pt-0">
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={pieChartData} cx="50%" cy="50%" outerRadius={80} innerRadius={50} dataKey="value" labelLine={false}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <Card className="border border-default-100 shadow-none">
          ...
          <CardBody className="pt-0">
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={monthly_trend} margin={{ top: 8, right: 8, left: -16, bottom: 0 }}>
                <defs>
                ...
                <YAxis tick={{ fontSize: 11 }} axisLine={false} tickLine={false} />
                <ReTooltip content={<ChartTooltip suffix="contributions" />} />
                <Area type="monotone" dataKey="count" stroke="#378ADD" strokeWidth={2} fill="url(#areaGrad)" dot={{ r: 3, fill: "#378ADD", strokeWidth: 0 }} activeDot={{ r: 4 }} />
              </AreaChart>
            </ResponsiveContainer>
          </CardBody>
        </Card>
              ...
              top_contributors.map((contributor, i) => (
                <div key={i} className="flex items-center gap-2.5 px-3 py-2 rounded-lg bg-default-50 hover:bg-default-100 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] font-medium text-foreground truncate">{contributor.name}</p>
                    <p className="text-[11px] text-default-400">{contributor.count} contribution{contributor.count !== 1 ? "s" : ""}</p>
                  </div>
                  {i < 3 && <LuTrophy size={14} style={{ color: trophyColor(i) }} />}
                </div>
              ))
            )}
          </CardBody>
      ...
      {/* ── Faculty Statistics ── */}
      <div>
        <h2 className="text-[15px] font-medium text-foreground mb-3">Faculty statistics</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {faculty_performance.map((faculty, i) => {
            const pct = faculty.total_contributions > 0
              ? Math.round((faculty.selected_count / faculty.total_contributions) * 100)
              : 0;
            const { bg, text } = CARD_COLORS[i % CARD_COLORS.length];
            const isDownloading = loadingFaculty[faculty.id] ?? false;

            return (
              <Card key={faculty.id} className={`${bg} ${text} border-0 overflow-hidden relative`}>
                ...
                <CardBody className="pt-0 flex flex-col gap-3 relative z-10">
                  ...
                  <div className="flex flex-col gap-1">
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] opacity-65">Selection rate</span>
                      <span className="text-[10px] font-medium opacity-90">{pct}%</span>
                    </div>
                  ...
                  <div className="flex divide-x divide-white/20">
                    {[
                      ...
                      <div key={label} className="flex-1 flex flex-col gap-0.5 px-2.5 first:pl-0 last:pr-0">
                        <span className="text-[17px] font-medium leading-none">{value}</span>
                  ...
                  <Button
                    ...
                    onPress={() => handleDownloadFaculty(faculty.id, faculty.name)}
